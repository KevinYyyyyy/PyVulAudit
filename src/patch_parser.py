...
from src.constant import (
    ...
    SCOPE_CACHE_DIR_DATE, AST_TYPE_CACHE_DIR_DATE
)
...
from data_collection.collect_changes import (
    ...
    find_minimal_enclosing_scope, filter_vulnerable_methods, get_methods
)
...
class CommitProcessor:
    ...
    def get_code_change_scope_for_all(self, cve_id: str, fixing_commits: List[str], 
        ...
        for fixing_commit in fixing_commits:
            ...
            if scope_cached.exists() and False:
                with open(scope_cached, 'rb') as f:
                    commit2file2scope = pickle.load(f)
            else:
                file2scope = {}
...
class PatchParser:
    ...
    def load_cve2advisory(self, force_update: bool = False) -> Optional[Dict[str, Dict]]:
        ...
        参考 vulnerability_collector.py 中的 fetch_vul_records_from_osv 方法
    ...
    def process_possible_commit_urls(self, cve2advisory: Dict[str, Dict], 
        ...
        if n_jobs > 1:
            ...
            results = Parallel(n_jobs=n_jobs,backend='threading')(
                delayed(self.commit_processor.process_cve_commits)(
            ...
            processed_commits = {}
            new_cve2advisory = {}
            for cve_id, cve_vfc_infos, advisory_copy in results:
                if len(cve_vfc_infos):
                    processed_commits[cve_id] = cve_vfc_infos
                    new_cve2advisory[cve_id] = advisory_copy
        else:
    ...
    def save_scope_and_vulnerability_analysis(self, scope_analysis: Dict[str, Dict], 
                                            vul_func_analysis: Dict[str, Dict]):
        """
        Save scope and vulnerability analysis results.
        ...
        try:
            # Save scope analysis
            scope_output_file = self.output_dir / f"scope_analysis_{COMMITS_DIR_DATE}.json"
            with open(scope_output_file, 'w', encoding='utf-8') as f:
                json.dump(scope_analysis, f, indent=2, ensure_ascii=False)
            logger.info(f"Saved scope analysis to {scope_output_file}")
            
            # Save vulnerable functions analysis
            vul_func_output_file = self.output_dir / f"vulnerable_functions_analysis_{COMMITS_DIR_DATE}.json"
            with open(vul_func_output_file, 'w', encoding='utf-8') as f:
                json.dump(vul_func_analysis, f, indent=2, ensure_ascii=False)
            logger.info(f"Saved vulnerable functions analysis to {vul_func_output_file}")
    ...
    def save_cves_has_vulnerable_functions(self, cve2advisory: Dict[str,Dict], vul_func_analysis: Dict[str, Dict]):
        ...
        # Check which CVEs have vulnerable functions
        for cve_id, cve_data in vul_func_analysis.items():
            ...
            for package_name, package_data in cve_data.items():
                ...
                # Check if any fixing commit has vulnerable functions
    ...
    def analyze_patches(self, cve2advisory: Dict[str, Dict], 
        ...
        Args:
            cve2advisory: Dictionary mapping CVE IDs to advisory information
            force_update: Whether to force update existing analysis
            filter_large_source: Whether to filter large PR/Issue sources
            filter_large_vfcs: Filter commits with too many changes
            priority_commit: Prioritize direct commit references
            filter_large_files: Filter commits with large file changes
        ...
        processed_commits = self.process_possible_commit_urls(
            cve2advisory, 
            rewrite_all_fixing_commits=force_update,
        ...
        scope_analysis = self.analyze_code_change_scope_for_all(filtered_commits, rewrite=force_update)
        
        # Step 4: Analyze vulnerable functions for all CVEs
        vul_func_analysis = self.analyze_vulnerable_functions_for_all(filtered_commits, scope_analysis, rewrite=force_update)
        ...
        return filtered_commits, changes_analysis
                ...
                for fixing_commit, commit_data in package_data.items():
                    ...
                    for file_id, scope_data in commit_data.items():
                        ...
                        for scope, changes in deleted_scope.items():
                            ...
                            # Track scope types for CVE-level statistics
                            if scope == 'function':
    ...
    def evaluate_vulnerable_functions(self, vul_func_analysis: Dict[str, Dict]) -> Dict[str, Any]:
        """
        评估vulnerable functions的统计信息
        
        Args:
            vul_func_analysis: vulnerable functions分析结果
                              Structure: CVE -> package -> {'repo_url': str, 'vulnerable_functions_data': Dict}
            
        Returns:
            Dict: 包含vulnerable functions统计结果的字典
        """
        logger.info("开始评估vulnerable functions...")
        cve_with_vf={}
        ...
        # 遍历vulnerable functions分析结果
        for cve_id, cve_data in tqdm(vul_func_analysis.items(), desc="评估vulnerable functions"):
            ...
            for package_name, package_data in cve_data.items():
                ...
                # 遍历每个fixing commit的vulnerable functions
                for fixing_commit, commit_data in vf_data.items():
                    total_vfcs += 1
        ...
        # 输出统计结果
        logger.info(f"\n=== Vulnerable Functions 总体统计 ===")
        logger.info(f"总CVE数: {total_cves}")
        logger.info(f"总包数: {total_packages}")
        logger.info(f"总VFC数: {total_vfcs}")
        logger.info(f"总vulnerable functions数: {total_vulnerable_functions}")
        
        logger.info(f"\n=== CVE级别统计 ===")
        logger.info(f"能提取到函数的CVE数: {cves_with_functions_count} ({cves_with_functions_count/total_cves*100:.1f}%)")
        logger.info(f"无法提取到函数的CVE数: {cves_without_functions_count} ({cves_without_functions_count/total_cves*100:.1f}%)")
        logger.info(f"平均每个CVE的函数数: {avg_functions_per_cve:.2f}")
        logger.info(f"有函数的CVE平均函数数: {avg_functions_per_cve_with_functions:.2f}")
        
        logger.info(f"\n=== VFC级别统计 ===")
        logger.info(f"能提取到函数的VFC数: {vfcs_with_functions_count} ({vfcs_with_functions_count/total_vfcs*100:.1f}%)")
        logger.info(f"无法提取到函数的VFC数: {vfcs_without_functions_count} ({vfcs_without_functions_count/total_vfcs*100:.1f}%)")
        logger.info(f"平均每个VFC的函数数: {avg_functions_per_vfc:.2f}")
        
        logger.info(f"\n=== 策略贡献统计 ===")
        for strategy, summary in strategy_summary.items():
            if summary['total_functions'] > 0:
                logger.info(f"{strategy}:")
                logger.info(f"  函数数: {summary['total_functions']} ({summary['function_percentage']*100:.1f}%)")
                logger.info(f"  覆盖CVE数: {summary['cves_count']} ({summary['cve_coverage_rate']*100:.1f}%)")
                logger.info(f"  覆盖VFC数: {summary['vfcs_count']} ({summary['vfc_coverage_rate']*100:.1f}%)")
        ...
        logger.info("Vulnerable functions评估完成")
        return result
    ...
    def filter_cves_by_vulnerable_functions(self, cve2advisory: Dict[str, Dict], 
        ...
        """
        根据vulnerable functions的数量和类型过滤CVE
        基于collect_changes.py中的filter_cves_by_vulnerable_functions函数
        
        Args:
            cve2advisory: CVE到advisory的映射
            min_functions: 最小vulnerable functions数量
            max_functions: 最大vulnerable functions数量
            min_vfcs: 最小VFC数量
            max_vfcs: 最大VFC数量
            required_categories: 必须包含的漏洞类别
            excluded_categories: 需要排除的漏洞类别
            
        Returns:
            Tuple[Dict, Dict]: 过滤后的CVE字典和统计信息
        """
        logger.info(f"开始过滤CVE，条件：min_functions={min_functions}, max_functions={max_functions}")
        ...
        for cve_id, advisory in tqdm(cve2advisory.items(), desc="过滤CVE"):
        ...
        logger.info(f"过滤完成：原始CVE数 {filter_stats['original_count']}, 过滤后CVE数 {filter_stats['filtered_count']}")
        logger.info(f"过滤原因统计：{dict(filter_stats['filter_reasons'])}")
...
def main():
    """
    Main function for testing patch parser functionality
    """
    # Initialize components
    parser = PatchParser()
    cve2advisory = parser.load_cve2advisory()
    # Collect vulnerabilities (this would normally be done by vulnerability_collector.py)
    
    # Limit to a small sample for testing
    sample_cves = list(cve2advisory.keys())[:]
    sample_advisory = {cve: cve2advisory[cve] for cve in sample_cves }

    # Analyze patches
    logger.info("Analyzing patches...")
    commits_analysis, changes_analysis = parser.analyze_patches(
        sample_advisory,
        force_update=False,
        filter_large_source=True,
        filter_large_vfcs=True,
        priority_commit=True,
        filter_large_files=True,
        n_jobs=1
    )
    #
    logger.info(f"Analysis completed for {len(commits_analysis)} CVEs")
    logger.info(f"Total commits analyzed: {sum(len(cve_commits) for cve_commits in commits_analysis.values())}")
