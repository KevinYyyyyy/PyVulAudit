...
def normalize_package_funcs(all_pairs,all_upstream_with_py_file,all_downstream_with_py_file):
    ...
    for cve_id, cve_data in all_pairs.items():
        ...
        for upstream, all_downstream in cve_data.items():
            ...
            if normalized_vulnerable_funcs_file.exists():
            ...
            if (pkg,version) in all_upstream_with_py_file: 
                ...
                filtered_python_files = EnvAnalyzer.find_project_py_files(
                    pkg, version, workdir=args.workdir
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process CVE vulnerability data')
    parser.add_argument('--size', choices=['small', 'medium', 'large'], 
                       default='small', help='Dataset size')
    parser.add_argument('--workdir', type=Path, default=Path('../docker_workdir_new'),
                       help='Working directory')
    parser.add_argument('--snapshot-dir', type=Path, default=DATA_DIR/SUFFIX/'snapshots/0926',
                       help='Snapshot directory containing dependency graphs')
    parser.add_argument('--threads', type=int, default=10,
                       help='Number of threads')
    parser.add_argument('--memory', type=int, help='Max memory per thread in GB')
    parser.add_argument('--no-use-cache', action='store_true', default=False,
                       help='Use cached results when available')
    
    args = parser.parse_args()
    args.use_cache = not args.no_use_cache
    ...
    cve2advisory = {k: v for k, v in cve2advisory.items() if k in active_cves}
    ...
    logger.info(f"Downstream: {sum(len(tasks) for tasks in install_tasks_for_downstream.values())} total tasks")
    logger.info(f"Upstream: {sum(len(tasks) for tasks in install_tasks_for_upstream.values())} total tasks")
    ...
    all_upstream_filtered_with_py_file ,all_downstream_filtered_with_py_file, all_pairs_filtered= normalize_package_funcs(all_pairs,all_upstream_with_py_file,all_downstream_with_py_file)
