...
class ExtProcessor(ProcessingBase):
    def __init__(
        ...
        self.stdlib_path = f"{sys.prefix}/lib/python{sys.version_info.major}.{sys.version_info.minor}"
        ...
        if filename.endswith(".so") or filename.endswith("unicode.py"):
            ...
            self.filename = None
            return
        # if modname in sys.builtin_module_names and skip_builtin:
        #     self.import_manager = None
        #     self.modules_analyzed = set()
        #     self.filename = None
        #     return
        #
        # if modname in sys.modules and sys.modules[modname].startswith(self.stdlib_path) and skip_builtin:
        #     self.import_manager = None
        #     self.modules_analyzed = set()
        #     self.filename = None
        #     return
    ...
    def pushStack(self, callDefi: Definition, isVisited=True, isEntry=False):
        ...
        #获取函数定义的行号范围
        lines = getattr(callDefi, 'lines', None)
        if callNs in self.callStack:
            if gol.get_value('precision',False):
                self.cg.add_edge(self.current_ns, callNs, lines)
            return
        gol.add_value('cnt')
        ...
        if callDefi.get_type() == utils.constants.FUN_DEF:
            self.cg.add_edge(self.current_ns, callDefi.get_ns(), lines)
    ...
    def visit_Import(self, node, prefix="", level=0):
        ...
        for import_item in node.names:
            ...
            imported_name = self.import_manager.handle_import(src_name, level,skip_builtin=self.skip_builtin)
            print('imported_name:',imported_name,type(imported_name))
            ...
            if fname.endswith(".py") and not imported_name in self.modules_analyzed:
                print('imported_name:',imported_name)
                self.analyze_submodule(imported_name)
            if not gol.get_value('precision', False):
                # assert False,f"{self.current_ns} {imported_name}"
    ...
    def _get_full_qualified_name(self, node):
        # 生成类似 func(a, b=1, *args, **kwargs) 的字符串
        ...
        # print(node.name+f"({node.lineno})")
        # fn_name = node.name+f".({node.lineno},{node.end_lineno})"
        fn_name = node.name
        return fn_name
    def _handle_function_def(self, node, fn_name, fn_ns):
        ...
        # 新增：记录 signature、filename、lineno
        # print("debug...")
        # print(node.__dict__)
        # fn_def.signature = self._get_signature(node)
        # fn_def.filename = self.filename
        # fn_def.lineno = node.lineno
        # fn_def.end_lineno = node.end_lineno
        fn_def.lines = (node.lineno,node.end_lineno)
        ...
        # print(fn_def.get_ns(), utils.constants.RETURN_NAME, utils.join_ns(fn_def.get_ns(), utils.constants.RETURN_NAME))
        # assert False
        ...
        # print("fn_def.__dict__:",fn_def.__dict__)
        return fn_def
    ...
    def visit_FunctionDef(self, node):
        ...
        fn_def = self._handle_function_def(node, self._get_full_qualified_name(node), curNs)
            ...
            if decorator:
                if isinstance(node, ast.Name):
                    ...
                    if getattr(node, "id", None) and self.is_builtin(node.id):
                        ...
                        defi = self.def_manager.get(name)
                        if not defi:
                            self.def_manager.create(name, utils.constants.FUN_DEF,node)
                        return [name]
                    else:
    ...
    def analyze_submodule(self, imp):
        ...
        super().analyze_submodule(
            ...
            modules_analyzed=self.get_modules_analyzed(),
            decy=self.decy,
            skip_builtin=self.skip_builtin
        )
    ...
    def analyze(self):
        ...
        try:
            ...
            # raise ExtProcessorError("analyze error", e)
            pass
        # self.visit(ast.parse(self.contents, self.filename))
    ...
    def resolveList(self, calleeNs: str, row: int, flag=False) -> list:
        ...
        #     # return ['<map>']
            ...
            return [calleeNs]
class ExtProcessorError(Exception):
    pass