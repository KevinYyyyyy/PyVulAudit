...
class ImportManager(object):
    def __init__(self):
        ...
        self.all_external_modules = None
        self.all_external_pkgs = None
    ...
    def _do_import(self, mod_name, package):
        ...
        if module_spec is None:
            # TODO: load the corresponding package
            # 从自定义目录创建节点
            # if mod_name in self.all_external_modules:
            #     file_path = self.all_external_modules[mod_name][mod_name]
            #     self.create_edge(mod_name)
            #     self.create_node(mod_name)
            #     self.set_filepath(mod_name, file_path)
            #     return file_path
            # print(sys.path)
            return importlib.import_module(mod_name, package=package)
        # module_spec.loader.exec_module(mod_name)
    ...
    def get_version(self,mod_name):
        ...
        print(tmp)
        pass
    def handle_import(self, name, level,curScope=None,skip_builtin=False):
        ...
        if root in sys.builtin_module_names:
            ...
            print("name:",name, 'root in sys.builtin_module_names')
        ...
        # sys.path.append("downloads/numpy@1.8.1/sparselsh@1.1.3/sparselsh@1.1.3/numpy-1.8.1")
        # sys.path.append("downloads/numpy@1.16.0/airsimdroneracinglab@0.1.0/airsimdroneracinglab@0.1.0/numpy-1.16.0")
        ...
        for mn, pkg in combos:
            try:
                ...
                continue
                # raise ImportError("Attempting import no-existing module")
        print('MOD:', mod)

        if not mod:
            if mod_name.split('.')[0] in self.all_external_pkgs:
                print('Module "{}" not found.'.format(mod_name))
                assert False
            return

        if isinstance(mod, ModuleType) and (not hasattr(mod, "__file__") or not mod.__file__):
            print(hasattr(mod, "__file__"),mod.__file__)
            return
        # 如果是外部模块，则直接返回模块名
        if not isinstance(mod,str) :
            # print(self.mod_dir not in mod.__file__ and mod.__file__.startswith(self.stdlib_path), self.mod_dir, mod.__file__.startswith(self.stdlib_path))
            if self.mod_dir not in mod.__file__ and mod.__file__.startswith(self.stdlib_path):
                if skip_builtin and False:
                        # print('SKIP standard')
                    return
    ...
    def add_external_path(self, paths):
        """添加外部模块路径"""
        for path in paths:
        ...
        # 将path转化为pkg的名称
        # print('paths:',paths)
        self.all_external_pkgs = [path.split("/")[-2] for path in paths]
        # print("all_external_pkgs:",self.all_external_pkgs)
