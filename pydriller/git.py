...
class Git:
    ...
    def __init__(self, path: str, conf=None):
        ...
        self._conf.set_value("skip_whitespaces", True)
    ...
    def diff(self, from_commit_id: str, to_commit_id: str) -> List[ModifiedFile]:
        ...
        diff_index = from_commit.diff(
            ...
            create_patch=True,
            # 添加额外参数，例如：
            ignore_blank_lines=True,  # 忽略空行差异
            ignore_space_at_eol=True,  # 忽略行尾空格
        )
        return [ModifiedFile(diff=diff) for diff in diff_index]
