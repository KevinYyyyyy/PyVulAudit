...
class ModifiedFile:
    ...
    def get_code_changes_scopes(self):
        # print(self._is_source_code_file())
        # print(self.filename)
        # assert False
    ...
    def _is_source_code_file(self):
        ...
        #     return False
            ...
            return False
        elif 'setup.py'  in self.filename.lower() or 'setup.cfg'  in self.filename.lower():
            return False
        return True
        # if self.change_type == ModificationType.ADD or not self.old_path:
        #     return False
        ...
        # elif 'test'  in self.filename.lower():
        #     return False
        # elif 'setup.py'  in self.filename.lower():
        #     return False
        
        # elif self.filename.lower().endswith(tuple(exclude_suffixes)):
        #     return False
        # return True
    ...
    def cleaned_deleted_lines(self):
        deleted = self.diff_parsed["deleted"]
        print(self.diff_parsed["deleted"])
...
if __name__ == "__main__":
    ...
    # tree = parser.parse(bytes("processing_utils.check_all_files_in_cache(event_data._data)", "utf8"))
    # print
    # assert False
    # analyzer = CodeChangeAnalyzer(file_path='/Users/keviny/tmp/exp.py')
    # funcs = analyzer.get_functions()
    # print([func.long_name.replace('/.Users.keviny.tmp.','') for func in funcs])
    # print(len(funcs))
    # import json
    # with open('/Users/keviny/tmp/exp_cg.json', 'r') as f:
    #     cg = json.load(f)
    # methods = [node for node in cg.keys() if node.startswith('exp')]

    # print(methods)
    # print(len(methods))
    # # 查找确实的方法
    # for method in methods:
    #     if method not in [func.long_name.replace('/.Users.keviny.tmp.','') for func in funcs]:
    #         print(method)
    # print('-'*100)
    # for func in funcs:
    #     if func.long_name.replace('/.Users.keviny.tmp.','') not in methods:
    #         print(func.long_name.replace('/.Users.keviny.tmp.',''))

    analyzer = CodeChangeAnalyzer(file_path='/Users/keviny/tmp/global1.py',commit_hash='tmp',source_code=open('/Users/keviny/tmp/global1.py').read())
    print(analyzer.get_changed_vars_called_functions())

    analyzer = CodeChangeAnalyzer(file_path='/Users/keviny/tmp/global_real_case.py',commit_hash='tmp',source_code=open('/Users/keviny/tmp/global_real_case.py').read())
    print(analyzer.get_changed_vars_called_functions())